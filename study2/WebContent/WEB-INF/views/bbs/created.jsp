...
<script type="text/javascript">
  ...
  function sendBoard() {
        ...
        var mode="${mode}";
        if(mode=="created")
            f.action = "<%=cp%>/sbbs/created_ok.do";
        else if(mode=="update")
            f.action = "<%=cp%>/sbbs/update_ok.do";
  ...
  <table style="width: 600px; margin: 0px auto; border-spacing: 0px;">
     <tr height="45"> 
      <td align="center" >
        ...
        <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/sbbs/list.do?page=${mode == 'update' ? page : 1}';">${mode == "update" ? "수정취소" : "등록취소"}</button>
      </td>
    </tr>
  </table>
</form>
